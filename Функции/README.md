...
## Передача оригинала переменной (передача по ссылке)
> По умолчанию все аргументы передаются через копирование, то есть передается копия. Если копирование слишком трудоемко (например, передача изображения или большого файла) или нужно передать оригинал, чтобы как-то его изменить, используется оператор &, который нужно написать после типа данных аргумента.

Пример:
```c++
void sort3(float &a, float &b, float &c)
{
    // Тут у нас есть доступ к оригиналам переменных a, b, c.
    ...
}

void sort3__(float a, float b, float c)
{
    // Тут переменные a, b, c копируются.
    ...
}
```
...
3. (Самостоятельно): Описать функцию `addRightDigit(int, int)`, добавляющую к целому числу цифру из 2 аргумента справа. То есть `addRightDigit(123, 4)` должна вернуть 1234. Не делайте *овнокод, делайте без всяких to_string.
...
5. (ДЗ): Описать функцию `addLeftDigit(int, int)`, добавляющую к целому числу цифру из 2 аргумента слева. То есть `addLeftDigit(123, 4)` должна вернуть 4123. Не делайте *овнокод, делайте без всяких to_string.